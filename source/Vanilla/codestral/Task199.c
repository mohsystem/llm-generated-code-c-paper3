...
void addMinutes(Task199* clock, int minutes) {
    clock->hours = (clock->hours + (clock->minutes + minutes) / 60) % 24;
...
bool isEqual(Task199 clock1, Task199 clock2) {
...
int main() {
    ...
    printf("%d\n", isEqual(clock1, clock3)); // 0 (false)
    addMinutes(&clock1, 60);
    printf("%d\n", isEqual(clock1, clock3)); // 1 (true)
    subtractMinutes(&clock1, 120);
    printf("%d\n", isEqual(clock1, clock2)); // 1 (true)
    return 0;
}