...
THREAD_FUNC_T server_main(void *arg) {
    ...
    return (THREAD_FUNC_T)0;
}

// --- CLIENT COMPONENT ---

static void send_and_receive(SOCKET sock, const char* request, char* server_reply, size_t reply_cap) {
    char full_request[200];
    snprintf(full_request, sizeof(full_request), "%s\n", request);
    printf("Client sent: %s\n", request);
    if (send(sock, full_request, (int)strlen(full_request), 0) < 0) {
        puts("Send failed");
        return;
    }
    int recv_size = recv(sock, server_reply, (int)reply_cap - 1, 0);
    if (recv_size > 0) {
...
void run_client() {
    ...
    send_and_receive(sock, "LOGIN alice pass123", server_reply, sizeof(server_reply));

    printf("\nClient> Test 2: Incorrect password (bob)\n");
    send_and_receive(sock, "LOGIN bob wrongpass", server_reply, sizeof(server_reply));

    printf("\nClient> Test 3: User not found (dave)\n");
    send_and_receive(sock, "LOGIN dave somepass", server_reply, sizeof(server_reply));

    printf("\nClient> Test 4: Correct credentials (charlie)\n");
    send_and_receive(sock, "LOGIN charlie qwerty", server_reply, sizeof(server_reply));

    printf("\nClient> Test 5: Malformed request\n");
    send_and_receive(sock, "LOGI charlie qwerty", server_reply, sizeof(server_reply));

    // Signal server shutdown
    const char shutdown_msg[] = "SHUTDOWN\n";
    send(sock, shutdown_msg, (int)strlen(shutdown_msg), 0);
