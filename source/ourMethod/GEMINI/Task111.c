...
static xmlrpc_server_abyss_t * global_server;
...
void* server_thread_func(void* arg) {
    ...
    // Use the modern API to create the server.
    // xmlrpc_server_abyss_new_port(&env, registry, port, &global_server);
    die_if_fault_occurred(&env);
    ...
    // This part runs after the server is terminated from the main thread
    printf("Server has been shut down.\n");
    server_running = 0;

    // Clean up the server object.
    xmlrpc_server_abyss_destroy(global_server);
...
void run_client_tests(int port) {
    ...
    // Dynamically create the server URL based on the port.
    char server_url[100];
    snprintf(server_url, sizeof(server_url), "http://localhost:%d/RPC2", port);
...
int main() {
    ...
    xmlrpc_env env; // Needed for server termination call
    xmlrpc_env_init(&env);

    xmlrpc_client_setup_global_const(NULL);
    ...
    if (server_running) {
        ...
        // Use the modern terminate function signature (2 arguments).
        xmlrpc_server_abyss_terminate(&env, global_server);
        die_if_fault_occurred(&env);
    }
