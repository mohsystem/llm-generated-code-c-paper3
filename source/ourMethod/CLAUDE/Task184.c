...
// Uses rand() seeded with time and process info for better randomness
static bool generateUniqueName(char* buffer, size_t bufferSize) {
    ...
    // Note: For production use, use a CSPRNG like /dev/urandom
...
void robotInit(Robot* robot) {
    ...
    memset(robot->name, 0, NAME_LENGTH);  // Initialize to empty
}
