...
int reverseString(const char* input, char* output, size_t output_size) {
    ...
    /* Calculate input length safely using strnlen */
    input_len = strnlen(input, MAX_STRING_LENGTH + 1);

    /* Validate input length within allowed maximum */
    ...
    /* Reverse the string safely */
    for (i = 0; i < input_len; i++) {
        /* Prevent potential overflow — defensive coding */
        if (i >= output_size - 1) {
