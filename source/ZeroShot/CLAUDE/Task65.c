...
char* readFileSecurely(const char* filename) {
    ...
    if (strstr(filename, "..") != NULL || 
        ...
        strchr(filename, '\\\\') != NULL) {
        strcpy(errorMsg, "Error: Invalid filename format - path traversal detected");        return errorMsg;
    }
    // Validate filename characters\n
    if (!isValidFilename(filename)) {
        strcpy(errorMsg, "Error: Filename contains invalid characters");        return errorMsg;
    }   // Check if file exists and get stats\n
    struct stat fileStat;
    if (stat(filename, &fileStat) != 0) {        snprintf(errorMsg, sizeof(errorMsg), "Error: File does not exist: %s", filename);
        return errorMsg;   }   // Check if it's a regular file
