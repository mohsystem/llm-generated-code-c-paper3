...
char* base64_encode(const unsigned char* buffer, size_t length) {
    ...
    if (!b64 || !bmem) { BIO_free(b64); BIO_free(bmem); return NULL; }

    // No newlines in output
    BIO_set_flags(b64, BIO_FLAGS_BASE64_NO_NL);

    // Chain: b64 (top) -> bmem (sink)
    b64 = BIO_push(b64, bmem);

    if (BIO_write(b64, buffer, (int)length) <= 0) { BIO_free_all(b64); return NULL; }
    if (BIO_flush(b64) != 1) { BIO_free_all(b64); return NULL; }

    // Get pointer/length from the memory BIO (not from b64)
    char *out_ptr = NULL;
    long out_len = BIO_get_mem_data(bmem, &out_ptr);
    if (out_len <= 0 || !out_ptr) { BIO_free_all(b64); return NULL; }

    // Copy to a NUL-terminated string
    char *result = (char*)malloc((size_t)out_len + 1);
    if (!result) { BIO_free_all(b64); return NULL; }
    memcpy(result, out_ptr, (size_t)out_len);
    result[out_len] = '\0';

    BIO_free_all(b64); // frees both b64 and bmem
    return result;
}
