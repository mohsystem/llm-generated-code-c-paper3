...
/* helper: convert a single hex char to its value, or -1 if invalid */
...
static int from_hex_c(const char* hex, uint8_t* out, size_t out_len) {
    ...
    for (size_t i = 0; i < len / 2; i++) {
        int hi = hexval_char(hex[2 * i]);
        int lo = hexval_char(hex[2 * i + 1]);
        if (hi < 0 || lo < 0) return 0;
        out[i] = (uint8_t)((hi << 4) | lo);
    }
    return 1; /* keep original success return */
}
